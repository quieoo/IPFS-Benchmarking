...
func (c *datastoreConfig) Create(path string) (repo.Datastore, error) {
	//fmt.Printf("Create leveldb Datastore for %s\n",levelds.PrintCallerName())
	//called by github.com/ipfs/go-ipfs/repo/fsrepo.measureDatastoreConfig.Create
	p := c.path
	if !filepath.IsAbs(p) {
		p = filepath.Join(path, p)
	}
	//fmt.Printf("Create Levelds, compression: %s\n",c.compression)
