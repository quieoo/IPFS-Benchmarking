...
func (adder *Adder)GetAdder()string{
	var result string
	result=fmt.Sprintf("Progress:%t Pin:%t Trickle:%t RawLeaves:%t Silent:%t NoCopy:%t Chunker:%s\n",adder.Progress,adder.Pin,adder.Trickle,adder.RawLeaves,adder.Silent,adder.NoCopy,adder.Chunker)
...
func (adder *Adder) add(reader io.Reader) (ipld.Node, error) {
	...
	defer func() {
		quieoometrics.BufferCommitTIme+=time.Now().Sub(commitstart).Seconds()
	}()
...
func (adder *Adder) AddAllAndPin(file files.Node) (ipld.Node, error) {
	...
	//fmt.Printf("addFileNode time use %f\n",time.Now().Sub(start).Seconds()*1000)
	// get root
	rootflush:=time.Now()
	mr, err := adder.mfsRoot()
	if err != nil {
	...
	quieoometrics.RestInAddAll+=time.Now().Sub(rootflush).Seconds()
...
func (adder *Adder) addFile(path string, file files.File) error {
	start:=time.Now()
	defer func() {
		quieoometrics.AddfileTImeUse+=time.Now().Sub(start).Seconds()
	}()
