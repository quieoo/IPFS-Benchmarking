...
func (api *UnixfsAPI) Add(ctx context.Context, files files.Node, opts ...options.UnixfsAddOption) (path.Resolved, error) {
	//fmt.Printf("coreapi unixfs add   %s\n",time.Now())
	start:=time.Now()
	defer func() {
		quieoometrics.AddTimer.UpdateSince(start)
		quieoometrics.AddTimeUse +=time.Now().Sub(start).Seconds()
		quieoometrics.FileNumber+=0.001
		quieoometrics.AddBreakDownSummery()
	}()
	...
	//fmt.Printf("time to addallandpin %f ms\n",time.Now().Sub(start).Seconds()*1000)
	nd, err := fileAdder.AddAllAndPin(files)
	if err != nil {
		return nil, err
	}
	beginprovide:=time.Now()

	if !settings.OnlyHash {
		if quieoo.CMD_ImmeProvide{
			var cancel context.CancelFunc
			ctx, cancel = context.WithTimeout(ctx, time.Second*600)
			defer cancel()
			api.routing.Provide(ctx,nd.Cid(),true)
		}else{
	...
	quieoometrics.ProvideTimeUse +=time.Now().Sub(beginprovide).Seconds()
	//quieoometrics.AddBreakDownSummery()
	//fmt.Printf("provide time %f ms\n",time.Now().Sub(beginprovide).Seconds()*1000)
